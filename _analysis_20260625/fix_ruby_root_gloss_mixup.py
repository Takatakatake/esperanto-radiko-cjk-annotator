    ...
    for li, name in enumerate(LISTS):
        for idx, e in enumerate(d[name]):
            ...
            for i in hits:
                ...
                #   第100R: 値は文字列でもリストでも可。リストなら「そのいずれかと一致」で通す。
                #   (同じ語根が大小変種で `[词]ido` / `[词]Ido` / `[词]IDO` の3形を取るため。
                #    どれか1つだけを許すと `Feri[[词]Feri] ad[持续]` のような半端な状態が残る)
                g = t['guard']
                if g and g.get(lang) is not None:
                    gv = g[lang]
                    allowed = gv if isinstance(gv, list) else [gv]

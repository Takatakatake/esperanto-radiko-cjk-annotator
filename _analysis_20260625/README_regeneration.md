...
- audit_notation_equivalence.py … ★三表記入力(ĉ/cx/c^)等価性ゲート(第128R新設)。
  ...
  3言語同一 ②コーパス173本全文 ③実使用語彙22,259語、の3点。第128R初回=全PASS。
  ★既知の設計事実2件(据置・第128R): (a)真形にux等を含む外来固有名は正規化で変形する
  (コーパス実害はChaux-de-Fonds/Gembloux の2件のみ。xスラッグ等の他8件は正当変換)。
  (b)%...%保護は正規化の**後**に走るため表記変形からは守れない(置換からは守る)。
  どちらも上流由来設計で、変更は入力規約の変更=ユーザー裁定事項。

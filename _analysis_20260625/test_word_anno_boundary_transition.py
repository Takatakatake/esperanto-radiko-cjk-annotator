...
class WordAnnoBoundaryTransitionTests(unittest.TestCase):
    ...
    R88_ADDED_KEYS = set(r88_mukoz_policy.morph_context_annotations())

    def test_successor_parent_is_candidate_plus_phase619_sidecar(self):
        # The active manifest has since advanced through the independently
        # sealed ccb9398 transition.  Preserve this older link in the chain by
        # checking the successor's declared parent rather than pretending the
        # historical Phase 619 state is still active.
        active = json.loads(

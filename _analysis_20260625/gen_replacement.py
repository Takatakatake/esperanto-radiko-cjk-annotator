...
def generate(app_module_dir, data_dir, csv_path, stemming_json_path,
    ...
    imported_placeholders_for_global_replacement = import_placeholders(data_dir + r'\占位符(placeholders)_$20987$-$499999$_全域替换用.txt')
    imported_placeholders_for_2char_replacement = import_placeholders(data_dir + r'\占位符(placeholders)_$13246$-$19834$_二文字词根替换用.txt')
    imported_placeholders_for_local_replacement = import_placeholders(data_dir + r'\占位符(placeholders)_@20374@-@97648@_局部文字列替换用.txt')

    with open(lp(data_dir + r'\Unicode_BMP全范围文字幅(宽)_Arial16.json'), 'r', encoding='utf-8') as fp:
    ...
    # pre_replacements_dict_1 (sequential)
        ...
        if len(j) == 2:
            if len(j[0]) >= 2:
                if j[0] in pre_replacements_dict_1:
                    ...
                    _ns = j[0].replace('/', '')
                    if word_anno_nosl is not None and _ns in word_anno_nosl:
                        ruby = build_ruby_from_anno(word_anno_nosl[_ns])
                    else:
                        ruby = safe_replace(j[0], temporary_replacements_list_final)
                    pre_replacements_dict_1[j[0]] = [ruby, j[1]]

...
def generate(app_module_dir, data_dir, csv_path, stemming_json_path,
    ...
    for i in custom_stemming_setting_list:
        if len(i) == 3:
            try:
                ...
                if (not _setting_wants_split) and _wa_usable:
                    ...
                    _parts=[]
                    for _pc in [q for q in i[0].split('/') if q]:
                        if _pc in _GRAM_ENDINGS:
                            _parts.append(_pc); continue
                        # 動詞語尾片は素通し(後段の2char接尾機構が標準ルビ化: as=現在形等)。
                        # word_anno単体の偽友(as=エース等)を拾わないため。
                        if _pc in ('as','is','os','us','i','u'):
                            _parts.append(_pc); continue
                        # 片がword_annoに単体1ユニットで存在 → その注釈でルビ化(固有名詞・借用語根)
                        _pa = word_anno_nosl.get(_pc) if word_anno_nosl is not None else None
                        ...
                        if _m:
                            _parts.append(_r)
                        elif _pc in _csv_root_map:
                            _parts.append(output_format(_pc, _csv_root_map[_pc], format_type, char_widths_dict))
                        else:
                            _parts.append(_pc)
    ...
    pre_replacements_list_2 = sorted(pre_replacements_list_1,
                                     key=lambda x: (x[2], len(x[0]), _is_important(x[0])), reverse=True)

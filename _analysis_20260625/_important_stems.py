...
def _destem(w):
    ...
    for s in _GSUF:
        if w.endswith(s) and len(w) - len(s) >= 2:
            return w[:-len(s)]
    return w
# 候補を優先順に探索(第48Rでパス切れを修復: 旧20260621ディレクトリ移行後、静かに0語幹を返していた)
_ROOT2890 = os.path.join(r"D:" + os.sep, "GoogleDrive202510", "マイドライブ", "20_エスペラント・語学", "エスペラントの漢字化プロジェクト総結集20260630")
_CSV_CANDIDATES = [
    os.path.join(_ROOT2890, "エスペラント辞書徹底語根分解_20260630",
                 "2890 Gravaj Esperantaj Vortoj kun Signifoj en la Japana, Ĉina kaj Korea_260505_plej_nova.csv"),
    os.path.join(_ROOT2890, "エスペラント語根＿漢字割り当て＿20260630", "30_重要語彙CSV_日中対照_2890語",
                 "2890 Gravaj Esperantaj Vortoj kun Signifoj en la Japana, Ĉina.csv"),
]
CSV_2890 = next((c for c in _CSV_CANDIDATES if os.path.exists(lp(c))), _CSV_CANDIDATES[0])
def load_important_stems(path=CSV_2890):
    out = set()
    if not os.path.exists(lp(path)):
        print(f'[警告] 2890重要語CSVが見つかりません: {path} → important_stems空(タイブレーク無効)')
        return out

# -*- coding: utf-8 -*-
"""
エスペラント関連資料を外部SSD(E:)に日付付きでバックアップ。
backup_to_downloads.py の外部SSD版。再利用可: python backup_to_ssd.py
"""
import os, subprocess, datetime, sys
sys.stdout.reconfigure(encoding="utf-8")

date = datetime.date.today().strftime("%Y%m%d")
SSD_ROOT = r"E:\エスペラント_外部SSDバックアップ_" + date
os.makedirs(SSD_ROOT, exist_ok=True)
...
if not os.path.exists("E:\\"):
    print("E: ドライブ未接続。中止。"); sys.exit(1)

for name, path in sources:
    tgt = os.path.join(SSD_ROOT, name)
    print(f"==== {name} ====")

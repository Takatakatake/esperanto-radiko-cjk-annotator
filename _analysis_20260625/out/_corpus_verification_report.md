...
## 自動補正(先頭1字孤立)組込み後の最終測定 (2026-06-29, _autofix_perdoc_final.py)
- 全122文書 境界一致: baseline 最低97.1%/中央99.6% → **autofix 最低97.1%/中央99.6%(分布不変・悪化文書ゼロ)**。
- 全体: baseline 99.4%(107175) → **autofix 99.4%(107181, +6token, 回帰ゼロ)**。
- 子音始まり孤立 46種のうち **非ハイフンの真欠陥17種を全修正**(sporti→sport/i, korlig→kor/lig, fero→fero(=鉄) 等)。
- 残存29種は**全てハイフン付き**で、大半は欠陥でない:
  - 正当な略号プレフィックス(先頭1字分離が正しい): `v-tubisto`(V-tuber), `k-popo`(K-pop), `x-konto`, `z-generacio`, `d-ron`/`s-ron`(D-ro/S-ro)。
  - ハイフン付き固有名詞・外来語(`chopin-instituto`, `walter-familio`, `hirosima-nagasaki`)＋`--城市--城市--`連結トークン(解析異常)。
  - autofixは**ハイフン付きを意図的にスキップ**(正当な略号を壊さない安全設計)。
